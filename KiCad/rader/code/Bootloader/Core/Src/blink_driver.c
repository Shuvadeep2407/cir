/**
  ******************************************************************************
  * @file    blink_driver.c
  * @brief   Timing, CRC32, Flash, NVM, and System utilities - LL-based
  ******************************************************************************
  */
...
/**
  * @brief  Initialize SysTick for 1ms timing
  */
void Tick_Init(void)
{
...
/**
  * @brief  SysTick increment (called from SysTick_Handler)
  */
void Tick_Increment(void)
{
    g_tick++;

    /* Update non-blocking LED patterns/blinks */
    LED_Update();
}
...
{
    /* Enable IWDG (LSI ~32kHz) */
    IWDG->KR = 0x5555;  /* Enable write access */
    
    /* Calculate prescaler and reload for desired timeout */
    /* IWDG counter is 12-bit (max 4095) */
    /* Prescaler: 4 -> 0.125ms per tick at 32kHz */
    uint32_t reload = (timeout_ms * 8) - 1;  /* ~0.125ms per tick */
    if (reload > 4095) reload = 4095;
    
    IWDG->PR = IWDG_PR_PR_0;  /* Prescaler /4 */
    IWDG->RLR = reload;
...
/**
  * @brief  Calculate CRC32 over data buffer
...
/**
  * @brief  Calculate CRC32 over flash region
...
int Flash_Write(uint32_t address, const uint8_t* data, uint32_t len)
{
    ...
    for (uint32_t offset = 0; offset < len; offset += 8U)
    {
        ...
        memcpy(&value, data + offset, copy_len);
...
/**
  * @brief  Write configuration to NVM (last 2KB of flash)
  * @param  id: Configuration ID (0-255)
  * @param  data: Data to write
  * @param  len: Length
  * @retval 0 on success
  */
...
{
    if (id > 255 || len > 8) return -1;
    
    uint32_t addr = NVM_BASE + (id * 8);
    if (addr + len > NVM_BASE + NVM_SIZE) return -1;
    
    /* Erase page if writing to first entry */
    if (id == 0)
    {
        Flash_ErasePage(NVM_BASE);
    }
...
/**
  * @brief  Read configuration from NVM
  * @param  id: Configuration ID (0-255)
  * @param  data: Output buffer
  * @param  len: Length to read
  * @retval 0 on success
...
/**
  ...
  * @param  app_address: Application base address
  */
